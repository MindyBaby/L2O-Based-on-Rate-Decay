...
class Learner( object ):
    ...
    def __call__(self, num_roll=0) :  #全局训练
        ...
        if optimizee == 'SGD':
            ...
            optimizee = torch.optim.SGD([x], lr=0.01)
...
for loop_count in range(1):  
    ...
    plt.plot(x, adagrad_losses_tensor.numpy(), label='AdaGrad', linestyle=linestyles[3], marker=markers[3], markersize=6, color=colors[3], markeredgewidth=0, markevery=10)
    plt.plot(x, rms_losses_tensor.numpy(), label='RMSprop', linestyle=linestyles[1], marker=markers[1], markersize=6, color=colors[1], markeredgewidth=0, markevery=10)
    plt.plot(x, adam_losses_tensor.numpy(), label='Adam', linestyle=linestyles[2], marker=markers[2], markersize=6, color=colors[2], markeredgewidth=0, markevery=10)
